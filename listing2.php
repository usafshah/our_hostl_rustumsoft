...
?>
<style type="text/css">
  .usaf{ pointer-events: none;cursor: default;opacity: 0.6; } 
</style> 
<script>
function hostel(abc){
val=abc.split(',');
if(val[1]==1){
  window.open('listing2.php?user_query='+val[0]+'&type=flats','_SELF');
  }else{ window.open('listing2.php?user_query='+val[0],'_SELF'); } }
</script>
<?php 
 if (isset($_POST['district'])) {
 $sql = "SELECT * FROM district WHERE id=".$_POST["district"]."";  
 $result = mysqli_query($con, $sql); 
 $row=mysqli_fetch_array($result);
 $user_search= $row['name'];
}else{  $user_search=$_GET['user_query'];  }
?>
  ...
  <div class="container p-1"><!--Listing Started-->
        ...
        <div class="search">
         <div class="col-md-4"><span>Search result for:&nbsp<?php echo $user_search; ?></span></div>
         <div class="col-md-2">
          <select class="form-control" style="padding: 0px; margin-top:-20px;" onchange="hostel(this.value);">

            <option value="<?php echo $user_search; ?>,0" selected="selected" >Hostel</option>
            <option value="<?php echo $user_search; ?>,1" <?php echo (isset($_GET['type'])?'selected':''); ?>>Flats</option>
            </select>
      </div>
      ...
      <div class="clearfix"></div>
        ...
        if (!isset($_GET['type'])) {
/*pageination start*/
$total = "select * from hostel WHERE address LIKE '%$user_search%' && hostel_type!=3  && status!=0";
...
?>
  ...
  echo '<div id="paging"> <ul class="pagination">
    <li>'. $prevlink1. '</li>
    <li>'. $prevlink2. '</li>';
  echo ($page == 1) ?'<li><a href="?page=1&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >1</a></li>':'<li><a href="?page=1&user_query='.$_GET['user_query'].'" >1</a></li>';
  echo (2 <= $pages) ?($page == 2) ?'<li><a href="?page=2&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >2</a></li>':'<li><a href="?page=2&user_query='.$_GET['user_query'].'" >2</a></li>':'';
  echo (3 <= $pages) ?($page == 3) ?'<li><a href="?page=3&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >3</a></li>':'<li><a href="?page=3&user_query='.$_GET['user_query'].'" >3</a></li>':'';
  echo (4 <= $pages) ?($page == 4) ?'<li><a href="?page=4&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >4</a></li>':'<li><a href="?page=4&user_query='.$_GET['user_query'].'" >4</a></li>':'';
  echo (5 <= $pages) ?($page == 5) ?'<li><a href="?page=5&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >5</a></li>':'<li><a href="?page=5&user_query='.$_GET['user_query'].'" >5</a></li>':'';
...
$stmt1 = 'SELECT * FROM flats WHERE address LIKE "%'.$user_search.'%"  && status!=0 LIMIT '.$limit.' OFFSET '.$offset;
       $stmt=mysqli_query($con,$stmt1);
...
<?php    
  echo '<div id="paging"> <ul class="pagination">
    <li>'. $prevlink1. '</li>
    <li>'. $prevlink2. '</li>';
  echo ($page == 1) ?'<li><a href="?page=1&type=flats&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >1</a></li>':'<li><a href="?page=1&type=flats&user_query='.$_GET['user_query'].'" >1</a></li>';
  echo (2 <= $pages) ?($page == 2) ?'<li><a href="?page=2&type=flats&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >2</a></li>':'<li><a href="?page=2&type=flats&user_query='.$_GET['user_query'].'" >2</a></li>':'';
  echo (3 <= $pages) ?($page == 3) ?'<li><a href="?page=3&type=flats&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >3</a></li>':'<li><a href="?page=3&type=flats&user_query='.$_GET['user_query'].'" >3</a></li>':'';
  echo (4 <= $pages) ?($page == 4) ?'<li><a href="?page=4&type=flats&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >4</a></li>':'<li><a href="?page=4&type=flats&user_query='.$_GET['user_query'].'" >4</a></li>':'';
  echo (5 <= $pages) ?($page == 5) ?'<li><a href="?page=5&type=flats&user_query='.$_GET['user_query'].'" style="cursor: default;background-color: #008000;" title="Current Page" >5</a></li>':'<li><a href="?page=5&type=flats&user_query='.$_GET['user_query'].'" >5</a></li>':'';
